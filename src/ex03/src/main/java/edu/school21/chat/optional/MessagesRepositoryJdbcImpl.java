...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void save(Message message) throws NotSavedSubEntityException {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            SQLException e = null;
            if (affectedRows == 0) {
                throw new NotSavedSubEntityException("Failed to save message, no rows affected.", e);
            }
            try (ResultSet generatedKeys = preparedStatement.getGeneratedKeys()) {
                if (generatedKeys.next()) {
                    message.setId(generatedKeys.getLong(1));
                } else {
                    throw new NotSavedSubEntityException("Failed to save message, no ID obtained.", e);
                }
            }
        } catch (SQLException e) {
    ...
    public void update(Message message) throws NotSavedSubEntityException {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            throw new NotSavedSubEntityException("Failed to update message.", e);
        }
    }
